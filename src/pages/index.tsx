...
export const BigTitle = ({ children }: BigTitleProps) => (
  <h1 className="text-4xl sm:text-5xl lg:text-6xl font-sans font-medium text-white mb-6 tracking-wide">
    <div className="hidden min-[440px]:block">Hello,</div>
    <div className="hidden min-[440px]:block">
      <span className="inline whitespace-pre">I'm </span>
      <NameText />.
    </div>

    <div className="block min-[440px]:hidden">Hello, I&apos;m</div>
    ...
    >
      <div className={`w-full h-full `}>
        <div className="flex items-center">
          <div className="text-white uppercase text-2xl md:text-3xl xl:text-4xl tracking-wide font-mono">
            {title}
          </div>
        ...
        <div className="opacity-75 font-sans text-sm md:text-base">
          {subtitle}
        </div>
      </div>
    </a>
...
const ProjectCard = ({
  ...
  const onClickHandler = () => {
    setOpen((current) => checkId(current))
  }
  return (
    <div className="group perspective cursor-pointer" onClick={onClickHandler}>
      ...
      <div
        className={`w-full h-full shadow-lg rounded-lg overflow-hidden group-hover:rotate3d-x-10 2xl:group-hover:rotate3d-x-5 duration-200`}
        style={{ backgroundColor: bgColor }}
      >
        {cardImage && (
          <GatsbyImage
            className="bg-neutral-500 before:bg-black/80 before:absolute before:inset-0 before:z-10"
            image={cardImage}
            alt={title}
          />
        )}
        <div className="px-4 py-4">
          <h3 className="text-lg uppercase font-mono font-medium text-white">
            {title}
          </h3>
          <p className="text-sm font-sans font-light text-white leading-3">
            {subtitle}
          </p>
          ...
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center">
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-neutral-800 shadow-xl transition-all w-full max-w-4xl">
                <div>
                  <div
                    className="absolute inset-0 z-10 ml-auto mt-4 mr-4 flex h-12 w-12 items-center justify-center rounded-full bg-neutral-900/60 cursor-pointer"
                    onClick={onCloseHandler}
                  >
                    <XMarkIcon
                      className="h-6 w-6 text-neutral-100/80"
                      aria-hidden="true"
                    />
                  </div>
                  ...
                  <div className="my-6 justify-center mx-auto px-6 sm:px-32">
                    <Dialog.Title
                      as="h3"
                      className="text-3xl uppercase font-mono font-medium text-white"
                    >
                      {project?.name ?? ""}
                    </Dialog.Title>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 text-white">
            {skills.map((skill: string, index: number) => (
              <SkillCard
                key={index}
                name={skill}
                bgColor={skillPillBgColors[index]}
              />
            ))}
          </div>
        </div>
